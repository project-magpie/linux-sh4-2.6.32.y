...
mali_io_address _mali_osk_mem_allocioregion( u32 *phys, u32 size )
{
...
#if defined(__sh__)
	{
		...
		int i;
		/*
		* Make sure there are no dirty lines in either the L1 or L2
		* cache from a previous use of the pages by another Linux
		* subsystem. Also we need to set each page as reserved before
		* we can ioremap it.
		*/
		for (i=0;i < (1<<page_order); i++, tmp++)
		{
			flush_ioremap_region(page_to_phys(tmp), page_address(tmp), 0, PAGE_SIZE);
			SetPageReserved(tmp);
		}
...
#if defined(__sh__)
		/*
		 * flush_dcache_page will not flush the L2 on the SH4
		 */
		flush_ioremap_region(page_to_phys(new_page), page_address(new_page), 0, PAGE_SIZE);
		_mali_osk_mem_barrier();
#else
